...
function checkReferer($S,$protocol="http"){
	...
	*/
	if(!dirname($S)||dirname($S)=="."){
		$S=$protocol."://".$HTTP_HOST.dirname($SCRIPT_NAME)."/".$S;
	}
	...
	if($table_id_value){
		...
		foreach($table_columns as $table_field_name){
			...
			if(!$var_check||$var_isset){
				...
				if($table_field_value!=="" && !is_null($table_field_value)){
...
function jp7_db_update($table,$table_id_name,$table_id_value,$fields){
	...
	$rs=mysql_query($sql,$db)or die(jp7_debug(mysql_error()));
...
function interadmin_query($sql,$sql_db="",$sql_debug=false,$numrows=null,$offset=null){	
	...
	if(count($out[1])){
		...
		foreach($out[1] as $key=>$value){
			...
			elseif(strpos($value,$db_prefix.$lang->prefix."_arquivos")!==false||strpos($value,$db_prefix."_arquivos")!==false)$sql_where=str_replace("WHERE ","WHERE ".$alias.".mostrar<>'' AND (".$alias.".deleted_tipo='' OR ".$alias.".deleted_tipo IS NULL) AND ",$sql_where);
...
// interadmin_tipos_nome (2007/05/16 by JP)
 ...
 *
 */  
function jp7_debug($msgErro=null, $sql=null){
	...
	//if($GLOBALS['c_server_type']=="Principal"){
		...
		if($GLOBALS['c_server_type']=="Principal"){
			$S = "Ocorreu um erro ao tentar acessar esta página, se o erro persistir envie um email para <a href=\"jp7@jp7.com.br\">jp7@jp7.com.br</a>";
		}
	//} 
	return $S;	
}
...
$c_jp7=($REMOTE_ADDR=="201.6.156.3"||$REMOTE_ADDR=="192.168.0.2"||$REMOTE_HOST=="192.168.0.2"||$LOCAL_ADDR=="192.168.0.2"||$SERVER_ADDR=="192.168.0.2");
?>
