	...
	*/
	function xslt_error_handler( $parser, $errno, $level, $fields ) {
		...
		foreach( $msg as $label => $details ) {
			$error_details .= '<tr><td><b>' . $label . ': </b></td><td>' . htmlentities( $details , ENT_COMPAT | ENT_HTML401, 'ISO-8859-1') . '</td></tr>' . "\n";
		}
	...
	function ExtractSchema( $data = FALSE, $indent = '  ', $prefix = '' , $stripprefix=false) {
		...
		if( is_array( $tables = $this->db->MetaTables( 'TABLES' , ($prefix) ? $prefix.'%' : '') ) ) {
			foreach( $tables as $table ) {
				...
				$schema .= $indent . '<table name="' . htmlentities( $table , ENT_COMPAT | ENT_HTML401, 'ISO-8859-1') . '">' . "\n";
				...
				if( is_array( $fields ) ) {
					foreach( $fields as $details ) {
						...
						if( isset($details->has_default) && $details->has_default ) {
							$content[] = '<DEFAULT value="' . htmlentities( $details->default_value, ENT_COMPAT | ENT_HTML401, 'ISO-8859-1' ) . '"/>';
						}
						...
						$schema .= str_repeat( $indent, 2 ) . '<field name="' . htmlentities( $details->name, ENT_COMPAT | ENT_HTML401, 'ISO-8859-1' ) . '" type="' . $type . '"' . $extra;
				...
				if( is_array( $indexes ) ) {
					foreach( $indexes as $index => $details ) {
						...
						foreach( $details['columns'] as $column ) {
							$schema .= str_repeat( $indent, 3 ) . '<col>' . htmlentities( $column, ENT_COMPAT | ENT_HTML401, 'ISO-8859-1' ) . '</col>' . "\n";
						}
					...
					if( is_object( $rs ) && !$rs->EOF ) {
						...
						while( $row = $rs->FetchRow() ) {
							foreach( $row as $key => $val ) {
								if ( $val != htmlentities( $val, ENT_COMPAT | ENT_HTML401, 'ISO-8859-1' ) ) {
									$row[$key] = '<![CDATA[' . $val . ']]>';
								}
							}
...
function logMsg( $msg, $title = NULL, $force = FALSE ) {
	if( XMLS_DEBUG or $force ) {
		...
		if( isset( $title ) ) {
			echo '<h3>' . htmlentities( $title, ENT_COMPAT | ENT_HTML401, 'ISO-8859-1' ) . '</h3>';
		}
