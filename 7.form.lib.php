...
function interadmin_returnCampo($campo)
{
    ...
    if ($ajuda) {
        $S_ajuda = "<input type=\"button\" value=\"?\" tabindex=\"-1\" class=\"bt_ajuda\" onclick=\"alert('" . $ajuda . "')\" />";
    }
    if ($readonly == 'hidden') {
        $readonly_hidden = true;
    }
    if ($readonly || ($campo_array[permissoes] && $campo_array[permissoes] != $s_user['tipo'] && !$s_user['sa'])) {
        $readonly = ' disabled';
    }
    ...
    if (strpos($campo, 'tit_') === 0) {
        ...
        if ($xtra) {
            $form .= '<script>interadmin_iframes[' . $iframes_i . "]='" . $campo . '_' . $iframes_i++ . "'</script>";
        }
    ...
    if ($campo_nome) {
        if (strpos($campo, 'tit_') === 0) {
            ...
            if ($campo_array['thumb']) {
                echo '<img src="' . $campo_array['thumb'] . "\" alt=\"\" class=\"file_thumb\" />\r\n";
            }
            echo "</td>\r\n";
...
function interadmin_combo($current_id, $parent_type_id_2, $nivel = 0, $prefix = '', $sql_where = '', $style = 'select', $field_name = '', $field_label = '', $obrigatorio = '', $readonly = '')
{
    ...
    while ($row = $rs->FetchNextObj()) {
        if (is_array($current_id)) {
            $selected = in_array($row->id, $current_id);
        } else {
            $selected = ($row->id == $current_id);
        }
        if ($row->select_key && !in_array('select_key', $select_campos_2_array)) {
            if ($campos[select_key][xtra]) {
                $row->varchar_key = interadmin_tipos_nome($row->select_key);
            } else {
                $row->varchar_key = jp7_fields_values($row->select_key);
            }
        }
        // Combo Fields
        $select_campos_sql = '';
            ...
            foreach ($select_campos_2_array as $key => $value) {
                ...
                if ($select_campos_sql_temp) {
                    if (strpos($value, 'special_') === 0) {
                        ...
                        if (is_numeric($select_campos_sql_temp) && strpos($value, 'varchar_') === false && strpos($value, 'int_') === false) {
                            $select_campos_sql_temp = ($select_campos_2_xtra[$key]) ? interadmin_tipos_nome($select_campos_sql_temp) : jp7_fields_values($select_campos_sql_temp);
                        }
...
function jp7_DF_sendMail($post_vars, $from_info = false, $env_info = true, $attachments = '', $parameters = '')
{
    ...
    if (!$DF_client_name) {
        $DF_client_name = $DF_to_name;
    }
    if ($DF_cc_name) {
        $DF_cc = $DF_cc_name . ' <' . $DF_cc . '>';
    }
    if ($DF_bcc_name) {
        $DF_bcc = $DF_bcc_name . ' <' . $DF_bcc . '>';
    }
    if ($DF_template == 'default') {
        $DF_template = 'http://' . $_SERVER['HTTP_HOST'] . '/' . $GLOBALS['c_path'] . 'site/_templates/form_htm.php';
    }
    ...
    if ($DF_from) {
        if ($DF_from_name) {
            $headers .= 'From: ' . jp7_encode_mimeheader($DF_from_name) . ' <' . $DF_from . ">\r\n";
        } else {
            $headers .= 'From: ' . $DF_from . "\r\n";
        }
    }
    if ($DF_reply_to) {
    ...
    if ($DF_from && !$DF_reply_none) {
        ...
        if ($DF_client) {
            if ($DF_client_name) {
                $headers .= 'From: ' . $DF_client_name . ' <' . $DF_client . ">\r\n";
            } else {
                $headers .= 'From: ' . $DF_client . "\r\n";
            }
        }
        if ($DF_reply_message) {
            ...
            if ($DF_from_name) {
                $message = str_replace('%FROM_NAME%', $DF_from_name, $message);
            }
            if ($DF_client_name) {
                $message = str_replace('%CLIENT_NAME%', $DF_client_name, $message);
            }
        } else {
            if ($lang->lang == 'en') {
    ...
    if ($DF_redirect) {
        ...
        if ($debug) {
            echo 'Location: ' . $DF_redirect;
        } else {
            header('Location: ' . $DF_redirect);
        }
    } else {
        return $DF_send;
    }
...
function jp7_DF_prepareVars($db_prefix, $type_id, $vars_in, $var_prefix = '', $only_interadmin = false)
{
    ...
    foreach ($vars_in as $key => $value) {
        if (strpos($key, 'noDF_') === false) {
            // InterAdmin Vars
            if (
                strpos($key, $var_prefix . 'varchar_') !== false
                ...
                if (strpos($key, 'password_key_check') === false) {
                    $vars_out[$key] = $value;
                }
            }
            // All
            if (strpos($key, 'DF_') === 0 || !$only_interadmin) {
                $vars_out[$key] = $value;
            }
        }
    }
